...
public class TestSMILES2CMLContextualView {
	Activity<?> activity;
	
	@Before
	public void setup() throws ActivityConfigurationException {
		activity=new SMILES2CMLActivity();
		NoParametersConfigurationBean b=new NoParametersConfigurationBean();
		((SMILES2CMLActivity)activity).configure(b);
	}
	...
	public void testDisovery() throws Exception {ContextualViewFactory factory = ContextualViewFactoryRegistry.getInstance().getViewFactoryForObject(activity);
		assertTrue("Factory should be XMPPActivityViewFactory",
				factory instanceof SMILES2CMLActivityViewFactory);
		ContextualView view = factory.getView(activity);
		assertTrue("The view should be XMPPActivityContextualView",
				view instanceof SMILES2CMLActivityContextualView);
	}
	
	@Test
	public void testGetConfigureAction() throws Exception {
		ContextualView view = new SMILES2CMLActivityContextualView(activity);
		assertNotNull("The action should not be null",view.getConfigureAction(null));
		assertTrue("Should be a XMPPActivityConfigurationAction",
				view.getConfigureAction(null) instanceof SMILES2CMLActivityConfigurationAction);
	}
